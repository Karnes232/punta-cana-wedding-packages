...
- Light sand `#F5F1E8` or `#F9F6F2`
- Very light sky blue `#F0F4F8` or `#EBF2F5`
...
**Avoid Entirely**
❌ Black backgrounds or text
❌ Gold, rose gold, copper (luxury metals)
❌ Strong reds, dark reds, or burgundy
❌ Dark navy, dark blue, or purple
❌ Any dark/aggressive colors
...
- **Shadow:** Very light (optional subtle shadow)
...
- "Next Step"
- "See My Estimate"
- "Submit My Wedding Plan"
- "Talk to a Wedding Assistant"
- "View Pricing"
- "Explore Stories"
...
- **Sections:** Clear visual separation between sections (spacing or very light background color)
...
- Never use pure black; use very dark gray
...
- **Font size:** 14px–16px (readable, not tiny)
...
- **Bright environments:** Daylight, sunny, warm
...
- **Form validation:** Light fade-in of error message (150ms)
...
❌ Auto-playing videos (user should control)
...
[Generous padding top/bottom: 80px–120px]
  [Centered container, max-width: 1000px]
    ...
    [Generous space: 24px]
    [Primary CTA button]
[Generous padding bottom: 80px–120px]
```
...
[Padding: 60px–100px]
  [Container: max-width 1200px]
    [Grid: 2 columns with gap 40px–60px]
      [Column 1: Image or content]
      [Column 2: Text with strong hierarchy]
```
...
[Padding: 60px]
  [Container]
    [Grid: 3 columns (desktop), 1 column (mobile), gap 24px–32px]
      [Card with rounded corners, soft shadow]
        [Image: 100% width]
        [Padding inside: 24px–32px]
        [Heading]
        [Description]
...
4. **Ease of use** — Everything is intuitive

If a design choice doesn't serve one of these goals, reconsider it.
...
- **Color contrast:** Ensure text on backgrounds meets WCAG AA (4.5:1 for normal text)
- **Button size:** Minimum 44px × 44px for touch targets
- **Focus states:** Always visible (use border-blue or soft outline)
- **Labels:** Always label form fields (not just placeholder)
- **Alt text:** Describe every image
- **Motion:** Respect `prefers-reduced-motion` (avoid animations for users who opt out)

---

This is your visual north star. When in doubt, ask: "Does this feel calm, clear, and easy?" If not, redesign it.