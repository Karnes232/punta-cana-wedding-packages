...
function calculateTotal(state: CalculatorState): number {
  ...
  // Drinks
  total +=
    state.barPackage.cost_per_person_per_hour *
    state.barPackage.selected_hours *
    state.guests;
  ...
  total +=
    state.photo.cost +
    state.photo.add_ons.reduce((sum, addon) => sum + addon.cost, 0);
  total +=
    state.video.cost +
    state.video.add_ons.reduce((sum, addon) => sum + addon.cost, 0);
