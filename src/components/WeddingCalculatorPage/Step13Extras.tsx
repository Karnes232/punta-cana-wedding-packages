...
export default function Step13Extras({ state, dispatch, options }: Props) {
  ...
  const isSelected = (opt: ExtraOption) => state.extras.some((e) => e._id === opt._id)
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        {options.map((opt) => {
          ...
          const optionTotal = opt.isPerPerson ? opt.cost * state.guests : opt.cost
          return (
            <label
              key={opt._id}
              className={[
            ...
            >
              <input
                type="checkbox"
                checked={selected}
                onChange={() => dispatch({ type: 'TOGGLE_EXTRA', option: opt })}
              ...
              <div className="flex-1">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="shrink-0 text-right">
                    {opt.isPerPerson ? (
                      <>
                        <p className="text-xs text-[#888888]">{t('perPerson', { cost: formatUSD(opt.cost) })}</p>
                        <p className="text-sm font-semibold text-[#1A1A1A]">{formatUSD(optionTotal)}</p>
                      </>
                    ) : (
                      <p className="text-sm font-semibold text-[#1A1A1A]">{formatUSD(opt.cost)}</p>
                    )}
                  </div>
                </div>
                {opt.description && (
                  <p className="mt-1 text-xs leading-relaxed text-[#888888]">{opt.description}</p>
                )}
              </div>
            </label>
          )
      ...
      {extrasTotal > 0 && (
        <p className="mt-5 text-sm font-medium text-[#1A1A1A]">
          {t('total')} <span className="text-[#5B9FD9]">{formatUSD(extrasTotal)}</span>
        </p>
      )}
    </StepWrapper>
  )
