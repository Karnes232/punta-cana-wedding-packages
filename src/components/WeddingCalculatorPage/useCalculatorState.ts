...
function toggleItem<T extends { _key?: string; _id?: string }>(
  ...
  const id = (item as { _key?: string; _id?: string })._key ?? (item as { _id?: string })._id
  const exists = list.some(
    (x) => ((x as { _key?: string; _id?: string })._key ?? (x as { _id?: string })._id) === id,
  )
  return exists
    ? list.filter((x) => ((x as { _key?: string; _id?: string })._key ?? (x as { _id?: string })._id) !== id)
    : [...list, item]
}
...
function calculatorReducer(
  ...
  switch (action.type) {
    ...
    case 'TOGGLE_DECOR_ADDON':
      return { ...state, decorAddOns: toggleItem(state.decorAddOns, action.addon) }

    case 'SET_PHOTO':
      return { ...state, photo: action.photo, photoAddOns: [] }

    case 'TOGGLE_PHOTO_ADDON':
      return { ...state, photoAddOns: toggleItem(state.photoAddOns, action.addon) }

    case 'SET_VIDEO':
      return { ...state, video: action.video, videoAddOns: [], videoSkipped: false }

    case 'TOGGLE_VIDEO_ADDON':
      return { ...state, videoAddOns: toggleItem(state.videoAddOns, action.addon) }
...
export function calculateTotal(state: CalculatorState, config: CalculatorConfig): number {
  let total = 0
  ...
  const tableCount = Math.ceil(g / (state.furniture?.seatsPerTable ?? config.defaultSeatsPerTable))
...
export function useCalculatorState(config: CalculatorConfig) {
  ...
  return { state, dispatch, total, goToStep, SUMMARY_STEP, FORM_STEP, SUCCESS_STEP }
}
