...
import type { BarPackage, AddOn } from '@/sanity/queries/WeddingCalculator/getCalculatorData'
...
export default function Step06Bar({ state, dispatch, packages }: Props) {
  ...
  const barTotal = state.bar
    ? state.bar.costPerPersonPerHour * state.barHours * state.guests +
      state.barAddOns.reduce((sum, a) => sum + (a.isPerPerson ? a.cost * state.guests : a.cost), 0)
    : 0
  ...
  return (
    ...
    >
          ...
          return (
            ...
            >
              ...
              <p className="mt-1 text-xs text-[#888888]">
                {formatUSD(pkg.costPerPersonPerHour)}{t('perPersonPerHour')}
              </p>
              {pkg.description && (
      ...
      {state.bar && (
        <div className="mb-6">
          <p className="mb-2 text-sm font-medium text-[#333333]">{t('selectHours')}</p>
      ...
      {state.bar && (
        <p className="text-sm font-medium text-[#1A1A1A]">
          {t('barTotal')} <span className="text-[#5B9FD9]">{formatUSD(barTotal)}</span>
        </p>
      )}
    </StepWrapper>
  )
