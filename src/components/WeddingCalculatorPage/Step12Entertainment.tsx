...
export default function Step12Entertainment({ state, dispatch, options }: Props) {
  const t = useTranslations('weddingCalculator.steps.entertainment')

  const entertainmentTotal = state.entertainment.reduce((sum, e) => sum + e.cost, 0)
    ...
    >
          ...
          return (
            ...
            >
              <input
                type="checkbox"
                checked={selected}
                onChange={() => dispatch({ type: 'TOGGLE_ENTERTAINMENT', option: opt })}
      ...
      {entertainmentTotal > 0 && (
        <p className="mt-5 text-sm font-medium text-[#1A1A1A]">
          {t('total')} <span className="text-[#5B9FD9]">{formatUSD(entertainmentTotal)}</span>
