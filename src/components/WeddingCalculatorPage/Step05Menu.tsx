...
export default function Step05Menu({ state, dispatch, menus }: Props) {
  ...
  const selectedMenuCost = state.menu ? state.menu.costPerPerson * state.guests : 0
    ...
    >
            ...
            >
              <div className="flex items-start justify-between gap-2">
                ...
                <p className="shrink-0 text-sm font-semibold text-[#1A1A1A]">
                  {formatUSD(menu.costPerPerson)}{t('perPerson')}
                </p>
              </div>
              {menu.description && (
                <p className="mt-2 text-xs leading-relaxed text-[#888888]">{menu.description}</p>
              )}
              {menu.style && (
                <p className="mt-2 text-xs font-medium capitalize text-[#5B9FD9]">{menu.style}</p>
              )}
            </button>
          )
        })}
      ...
      {state.menu && (
        <p className="mt-5 text-sm font-medium text-[#1A1A1A]">
          {t('total')} <span className="text-[#5B9FD9]">{formatUSD(selectedMenuCost)}</span>
          <span className="ml-1 text-xs text-[#AAAAAA]">
            ({formatUSD(state.menu.costPerPerson)}/person × {state.guests} guests)
          </span>
        </p>
      )}
    </StepWrapper>
