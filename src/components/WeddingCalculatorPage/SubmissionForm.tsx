...
export default function SubmissionForm({ state, dispatch, config, total }: Props) {
  ...
  const seatsPerTable = state.furniture?.seatsPerTable ?? config.defaultSeatsPerTable
  const tableCount    = Math.ceil(state.guests / seatsPerTable)
  const vehicleCount  = state.hotel ? Math.ceil(state.guests / state.hotel.vehicleCapacity) : 0
  ...
  function buildSummary(): string {
    ...
    if (state.menu) lines.push(`Menu: ${state.menu.name} — ${formatUSD(state.menu.costPerPerson * state.guests)}`)
    if (state.bar)  lines.push(`Bar: ${state.bar.name} × ${state.barHours}h — ${formatUSD(state.bar.costPerPersonPerHour * state.barHours * state.guests)}`)
    if (state.furniture) lines.push(`Furniture: ${state.furniture.name} × ${tableCount} tables — ${formatUSD(state.furniture.costPerTable * tableCount)}`)
    if (state.decor)     lines.push(`Decor: ${state.decor.name} — ${formatUSD(state.decor.baseCost)}`)
    if (state.photo)     lines.push(`Photography: ${state.photo.name} — ${formatUSD(state.photo.cost)}`)
    if (state.video && !state.videoSkipped) lines.push(`Videography: ${state.video.name} — ${formatUSD(state.video.cost)}`)
    if (state.hotel && vehicleCount > 0)    lines.push(`Transportation: ${vehicleCount} vehicles — ${formatUSD(state.hotel.ratePerVehicle * vehicleCount)}`)
    if (state.entertainment.length > 0)     lines.push(`Entertainment: ${state.entertainment.map((e) => e.name).join(', ')}`)
    if (state.extras.length > 0)            lines.push(`Extras: ${state.extras.map((e) => e.name).join(', ')}`)
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm font-medium text-[#5B9FD9]">Estimated total: {formatUSD(total)}</p>
      </div>
      ...
      >
        ...
        {/* Name */}
        <div>
          <label className="mb-1.5 block text-sm font-medium text-[#333333]">{t('name')}</label>
          <input
            type="text"
            value={name}
        ...
        {/* Email */}
        <div>
          <label className="mb-1.5 block text-sm font-medium text-[#333333]">{t('email')}</label>
          <input
            type="email"
            value={email}
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
          <div>
            <label className="mb-1.5 block text-sm font-medium text-[#333333]">{t('whatsapp')}</label>
            <input
              type="tel"
              value={whatsapp}
          ...
          <div>
            <label className="mb-1.5 block text-sm font-medium text-[#333333]">{t('phone')}</label>
            <input
              type="tel"
              value={phone}
        ...
        {/* Notes */}
        <div>
          <label className="mb-1.5 block text-sm font-medium text-[#333333]">{t('notes')}</label>
          <textarea
            value={notes}
