    ...
    >
      <div className="max-w-lg">
        ...
        <div className="rounded-2xl border border-[#5B9FD9]/20 bg-gradient-to-br from-[#F0F7FF] to-[#FAFBFF] p-6">
          ...
          <p className="text-sm leading-relaxed text-[#555555]">{t('description')}</p>
        ...
        <label className="mt-5 flex cursor-pointer items-start gap-3">
          <input
            ...
            onChange={(e) => dispatch({ type: 'SET_VENUE_CONFIRMED', confirmed: e.target.checked })}
            className="mt-0.5 h-4 w-4 rounded border-[#E0E0E0] accent-[#5B9FD9]"
          />
          <span className="text-sm font-medium text-[#333333]">{t('confirm')}</span>
        </label>
      </div>
    </StepWrapper>
  )
