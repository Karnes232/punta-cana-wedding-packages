...
import type { VideoPackage, AddOn } from '@/sanity/queries/WeddingCalculator/getCalculatorData'
...
export default function Step10Video({ state, dispatch, packages }: Props) {
  ...
  return (
    <StepWrapper
      ...
      onSkip={() => { dispatch({ type: 'SKIP_VIDEO' }); dispatch({ type: 'NEXT_STEP' }) }}
    ...
    >
      ...
      {state.video && !state.videoSkipped && (
        <p className="text-sm font-medium text-[#1A1A1A]">
          {t('total')} <span className="text-[#5B9FD9]">{formatUSD(videoTotal)}</span>
        </p>
      )}
    </StepWrapper>
  )
