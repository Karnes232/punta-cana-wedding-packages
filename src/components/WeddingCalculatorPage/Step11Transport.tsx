        ...
        <div className="space-y-3 border-t border-[#5B9FD9]/20 pt-4">
          <div className="flex justify-between text-sm">
            <span className="text-[#666666]">
              {t("capacity", { n: state.hotel.vehicleCapacity })}
            </span>
            <span className="font-medium text-[#1A1A1A]">
              {formatUSD(state.hotel.ratePerVehicle)}
              {t("perVehicle")}
            </span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-[#666666]">
              {t("vehiclesNeeded", { n: vehicleCount, g: state.guests })}
            </span>
            <span className="font-medium text-[#1A1A1A]">
              {vehicleCount} × {formatUSD(state.hotel.ratePerVehicle)}
            </span>
          </div>
          <div className="flex justify-between border-t border-[#5B9FD9]/20 pt-3">
            <span className="font-semibold text-[#1A1A1A]">{t("total")}</span>
            <span className="text-lg font-semibold text-[#5B9FD9]">
              {formatUSD(transportTotal)}
            </span>
          </div>
        </div>
      </div>
