...
export default function Step07Furniture({ state, dispatch, options, defaultSeatsPerTable }: Props) {
  ...
  const furnitureTotal = state.furniture ? state.furniture.costPerTable * tableCount : 0
    ...
    >
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {options.map((option) => {
          ...
          return (
            <button
              key={option._id}
              onClick={() => dispatch({ type: 'SET_FURNITURE', furniture: option })}
              className={[
            ...
            >
              ...
              <p className="mt-1 text-sm font-medium text-[#888888]">
                {formatUSD(option.costPerTable)}{t('perTable')}
              </p>
              {option.description && (
                <p className="mt-2 text-xs leading-relaxed text-[#AAAAAA]">{option.description}</p>
              )}
      ...
      {state.furniture && (
        <p className="mt-5 text-sm font-medium text-[#1A1A1A]">
          {t('total')} <span className="text-[#5B9FD9]">{formatUSD(furnitureTotal)}</span>
          <span className="ml-1 text-xs text-[#AAAAAA]">
            ({formatUSD(state.furniture.costPerTable)}/table × {tableCount} tables)
          </span>
        </p>
      )}
    </StepWrapper>
