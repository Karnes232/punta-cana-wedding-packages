...
function Row({ label, value }: { label: string; value: string | null }) {
  ...
  return (
    <div className="flex items-start justify-between gap-4 py-3">
      ...
      <span className="shrink-0 text-sm font-medium text-[#1A1A1A]">{value}</span>
    </div>
  )
}
...
export default function SummaryView({ state, dispatch, config, total }: Props) {
  ...
  const barTotal = state.bar
    ? state.bar.costPerPersonPerHour * state.barHours * state.guests +
      state.barAddOns.reduce((s, a) => s + (a.isPerPerson ? a.cost * state.guests : a.cost), 0)
    : null

  const decorTotal = state.decor
    ? state.decor.baseCost + state.decorAddOns.reduce((s, a) => s + (a.isPerTable ? a.cost * tableCount : a.cost), 0)
    : null
  ...
  const videoTotal = state.video && !state.videoSkipped
    ? state.video.cost + state.videoAddOns.reduce((s, a) => s + a.cost, 0)
    : null

  const entertainmentTotal = state.entertainment.reduce((s, e) => s + e.cost, 0)
  const extrasTotal        = state.extras.reduce((s, e) => s + (e.isPerPerson ? e.cost * state.guests : e.cost), 0)

  const formattedDate = state.date
    ? new Date(state.date + 'T12:00:00').toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })
    : ''
    ...
    <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
      ...
      <div className="mb-8 rounded-2xl border border-[#EFEFEF] bg-white">
        <div className="border-b border-[#EFEFEF] px-6 py-4">
          <p className="text-xs font-semibold uppercase tracking-[0.1em] text-[#999999]">{t('breakdown')}</p>
        </div>
        <div className="divide-y divide-[#F5F5F5] px-6">
          <Row label={t('venue')} value={formatUSD(config.venueCost + config.coordinationCost)} />
          {state.menu && (
            <Row
          ...
          {state.bar && barTotal !== null && (
            <Row
              label={t('bar', { name: state.bar.name, h: state.barHours, n: state.guests })}
              value={formatUSD(barTotal)}
            />
          )}
          {state.furniture && (
            <Row
              label={t('furniture', { name: state.furniture.name, n: tableCount })}
          ...
          {entertainmentTotal > 0 && (
            <Row label={t('entertainment')} value={formatUSD(entertainmentTotal)} />
          )}
          {extrasTotal > 0 && (
        ...
        <div className="flex items-center justify-between rounded-b-2xl bg-[#5B9FD9]/5 px-6 py-5">
          ...
          <p className="text-2xl font-semibold text-[#5B9FD9] tabular-nums">{formatUSD(total)}</p>
        </div>
      </div>
