  ...
  const points =
    data?.whyPoints?.length
      ? data.whyPoints.map((p, i) => ({
          key: p._key,
          text: (locale !== 'en' ? p[locale as 'es'] : null) ?? p.en ?? FALLBACK_POINTS[i] ?? '',
        }))
      : FALLBACK_POINTS.map((text, i) => ({ key: String(i), text }));

  const imageUrl = data?.whyImage?.asset
    ? urlFor(data.whyImage.asset).width(800).height(600).fit('crop').auto('format').url()
    : null;
    ...
    <section className="bg-[#FAFAFA] py-16 md:py-20">
      <div className="mx-auto max-w-7xl px-6">
        <div className="grid grid-cols-1 items-center gap-12 md:grid-cols-2">
          {/* Text side */}
          <div>
            <h2 className="text-2xl font-semibold text-[#1A1A1A] md:text-3xl">{heading}</h2>
            <p className="mt-5 text-base leading-relaxed text-[#555555]">{body}</p>
            ...
            <ul className="mt-8 space-y-4">
              {points.map((point) => (
                <li key={point.key} className="flex items-start gap-3">
                  ...
                  <span className="text-sm leading-relaxed text-[#444444]">{point.text}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          {imageUrl ? (
            ...
            <div className="flex aspect-[4/3] w-full items-center justify-center rounded-2xl bg-[#EBF4FC]">
              <span className="text-sm text-[#5B9FD9]">Upload image via Sanity Studio</span>
            </div>
          )}
        </div>
      </div>
